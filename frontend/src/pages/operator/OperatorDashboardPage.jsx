    ...
    <div className="flex h-screen bg-brand-dark text-white overflow-hidden">
      ...
      <div className="w-64 border-r border-brand-border flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          <div className="text-xs text-brand-muted px-2 mb-1">
            Live Queue ({queue.length})
          </div>
          {queue.length === 0 && (
            <div className="text-xs text-brand-muted px-2">No active chats</div>
          )}
          {queue.map(conv => (
            <button
              key={conv.id}
        ...
        {/* Agent hours — pinned to bottom of sidebar */}
        <AgentHours />
